...
public:
  ...
  uint8_t begin(gpio_num_t resetPin = GPIO_NUM_NC, gpio_num_t mfioPin = GPIO_NUM_NC);
  ...
  uint8_t beginBootloader(gpio_num_t resetPin = GPIO_NUM_NC, gpio_num_t mfioPin = GPIO_NUM_NC);
...
private:
  ...
  bool _allocateWireBuffer();
  void _freeWireBuffer();
